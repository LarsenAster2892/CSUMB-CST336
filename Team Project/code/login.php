...
// $_SESSION["gameCart"] = $gameCart;
 ...
 $invalidPass	= NULL;
  ...
  function validator() {
    global $usernameR, $lastName, $firstName, $password, $email, $status, $includesPrefix, $firstTime ;
	  ...
	  //at least two characters are entered, spaces are acceptable.
	  if(!$usernameR || !preg_match("/^[a-z0-9_-]{3,15}$/", $usernameR)) {
			$status .= "<span style=\"color: #f00\">ERROR: You must enter a valid Username (3 to 15 characters with any lower case character, digit or special symbol “_-” only) </span> <br />";
			$invalidUN = "class='req'";
	  }
...
<title>Solid Shelf - Video Game Rentals</title>
  ...
  <?php echo '<link href="' .$cssPrefix . 'solid_shelf.css" rel="stylesheet" type="text/css">'; ?>
